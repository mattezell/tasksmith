...
- **Structured task log (JSONL)** — per-task append-only event log at `<workspace>/logs/task-<id>.jsonl`. Events: `task_start`, `iter_start`, `cc_complete`, `validation`, `iter_end`, `rate_limited`, `ejected`, `task_end`. Survives engine restarts. Enables post-mortem analysis without parsing console output.
- **Graceful iteration resume** — engine writes checkpoints to active task YAML after each iteration (iteration count + cumulative cost). On restart, tasks resume from the last completed iteration instead of starting over. Zero wasted tokens on re-running successful iterations.
- **Resume orphaned tasks on restart** — `engine.resumeActive()` scans `tasks/active/` on startup and returns orphaned tasks. Coordinator submits them to the worker pool automatically. Combined with iteration checkpointing, crashed tasks resume where they left off.
- **Skill injection into compiled prompt** — `assembleContext()` now resolves the SKILL.md matching `task.template` from three layers (project `.tasksmith` → workspace → global `~/.tasksmith`), strips frontmatter, replaces `$ARGUMENTS` with the task prompt, and injects the result. Different templates now produce meaningfully different prompts.
...
### Fixed
- **Approval gate state desynchronization** — CLI and REST API approve/reject previously moved files directly without clearing the coordinator's in-memory approval map or timeout timers. A task approved via CLI/API could still be auto-rejected when the stale timeout fired. Fix: REST API now receives an approval callback that routes decisions through `coordinator.handleApprovalDecision()`. CLI writes `.decision-<taskId>.json` files that the coordinator polls in its scan loop. Both paths now clear timers and in-memory state consistently.
- **Email provider was a stub** — `EmailProvider.send()` only logged a warning and returned false. Now implemented with optional `nodemailer` — dynamically imported at runtime. Install with `npm install nodemailer` to enable; graceful fallback with clear warning if not installed.
- **Bundled skills missing from npm package** — `installBundledSkills()` expected `.claude/skills/` in the package root but `package.json` `files` array didn't include it. Skills installed from source checkout but not from `npm install`. Added `.claude/skills/` to `files`.
- **`parseTask()` dropped DAG metadata** — `dag_id` and `depends_on` fields from task YAML were silently discarded during parsing. Tasks picked up from inbox lost their DAG association. Now preserved as `dagId` and `dependsOn` on the Task object.
- **MCP `submit_dag` used separate DAG manager** — MCP registered DAGs in its own local `dagManager` instance, disconnected from the coordinator's. Blocked tasks were never held or released. Now writes the full DAG file to inbox, where the coordinator's `prescanInboxForDAGs()` → `handleDAG()` flow handles registration, blocking, and task release correctly.
- **Worktree cleanup failed from non-repo cwd** — `removeWorktree()` ran `git worktree remove` without setting `cwd`, which fails when the process working directory is outside the git repo. Now derives the git root from the project directory and passes it as `cwd`.
- **Project-level skill discovery ignored `projectsDir`** — `invokeCC()` hardcoded `join(workspace, "projects", ...)` for project-level `.tasksmith` skill paths instead of using the resolved `projectsDir`. Custom `workspace.projectsDir` settings had no effect on skill discovery.
- **Non-default workspace skills not injected** — `invokeCC()` only added `~/.tasksmith` and project-level `.tasksmith` as `--add-dir` paths. If the active workspace differed (via `--dir` or `TASKSMITH_DIR`), skills installed there were never exposed to Claude Code. Now adds the workspace root as an `--add-dir` path, with deduplication when workspace equals global.
- **README compiled prompts list inaccurate** — listed "Template — task instructions with your prompt injected" as step 7. Updated to reflect actual skill injection: engine resolves SKILL.md for `task.template`, replaces `$ARGUMENTS`, and injects into the compiled prompt.
- **Email provider undocumented dependency** — README listed email as a supported outbound provider with no mention that it requires `nodemailer`. Now marked as requiring `npm install nodemailer`.
- **Plugin CLI command inconsistency in README** — plugin table used stale shorthand forms (`tasksmith docker`, `tasksmith pg`, etc.) that don't exist. Updated to actual `tasksmith plugin run <name>` form.
- **`workspace.projectsDir` ignored by runtime** — engine and MCP hardcoded `join(workspace, "projects")` in 8 locations, ignoring the configured `projectsDir`. Engine now resolves project paths through `resolveProjectsDir()`. MCP `list_projects` tool and project resources also use the configured directory.
- **MCP `submit_dag` bypassed dependency ordering** — wrote all DAG tasks to inbox immediately, allowing blocked tasks to execute before their dependencies completed. Now only writes root tasks (no dependencies) to inbox; blocked tasks are held by the DAG manager and released by the coordinator as predecessors complete.
- **Plugin hook transformations discarded** — `beforeTaskExecute` hook results from Docker and Proxmox plugins (injecting `_dockerContainer`, `_proxmoxVmId` params) were silently dropped. Coordinator now captures and applies the transformed task from `executeHooks()` before passing it to the engine.
- **REST API default bind address** — config and API server defaulted to `0.0.0.0` (all interfaces), contradicting README's claim of localhost-only. Changed to `127.0.0.1`. Users who need external access can explicitly set `host: "0.0.0.0"` in config.
